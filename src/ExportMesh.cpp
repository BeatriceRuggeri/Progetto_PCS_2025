...
namespace PolyhedralLibrary {
	...
	{
		ofstream file("CellODs.txt");
		if(!file.is_open())
		{
			cerr<<"error reading file Cell1Ds"<<endl;
			return; //break?
	    }
	file <<"Id;x;y;z\n";
	for(size_t i =0; i<mesh.Cell0DsId.size(); ++i)
	    {
			file << mesh.Cell0DsId[i] << ";" << mesh.Cell0DsCoordinates(0, i) << ";" << mesh.Cell0DsCoordinates(1, i) << ";"<< mesh.Cell0DsCoordinates(2, i) << "\n";
		}
	file.close();
	}
	void WriteCell1Ds(const PolyhedralMesh& mesh) 
	{
	 ...
	 if (!file.is_open()) 
	 {
       cerr << "error reading Cell1Ds" << endl;
       return;
     }
     file << "Id;start;end\n";
       for (size_t i = 0; i < mesh.Cell1DsId.size(); ++i) 
       {
            file << mesh.Cell1DsId[i] << ";" << mesh.Cell1DsExtrema(i, 0) << ";" << mesh.Cell1DsExtrema(i, 1) << "\n";
       }
        file.close();
    }
    void WriteCell2Ds(const PolyhedralMesh& mesh)
    {
		...
		if (!file.is_open()) {
        cerr << "error reading file Cell2Ds" << endl;
        return;
    }
    file << "Id;NumVertices;Vertices;NumEdges;Edges\n";
		...
		if (!file.is_open()) {
        cerr << "error reading Cell3Ds" << endl;
        return;
    	}
